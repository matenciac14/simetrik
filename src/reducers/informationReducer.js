...
export default function (state = initialState, action) {
    switch (action.type) {
        ...
        console.log(state.usuarios.filter( user => user.name.lastName.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 ))
            return{
                ...state,
                usuarios: state.usuarios.filter( user => user.name.lastName.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 ),
                ...
                fuentes: state.fuentes.filter( fuente => fuente.name.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 )
            };
        ...
        case START_SEARCH_DATE:
            return {
                ...
                tableros: state.tableros.filter( tablero => tablero.timestamp.createdAt.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 ),
                conciliaciones: state.conciliaciones.filter( conciliacion => conciliacion.timestamp.createdAt.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 ),
                fuentes: state.fuentes.filter( fuente => fuente.timestamp.createdAt.toLowerCase().indexOf(action.payload.toLowerCase()) !== -1 )
            };

        case START_SEARCH_NUMBER:
            return {
                ...state,
                usuarios: state.usuarios.filter( user => user.phone.indexOf(action.payload) !== -1 ),
